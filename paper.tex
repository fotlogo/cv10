...
%-------------------------------------------------------------------------------
\begin{abstract}
Attributes have been shown to be effective in assisting object classification, and they provide means to also incorporate semantic
...
regions can be combined together into larger regions with attribute
...
to be part of the object.  The other is subtractive, which first predicts attributes on
...
The high-level outline of our algorithm is shown in Figure
\ref{fig:flowchart}.
...
Attributes are high-level features which can reflect semantic properties of objects. Given the attributes, we can perform object classification based on an image's attribute distribution. We use the same attribute set as \cite{farhadi09}. This set contains three types of attributes, including shape, part and material. All attributes are listed in figure \ref{fig:atts}.
...
We use a similar approach to that used in \cite{farhadi09} to train the attribute models. 
...
the first branch in the tree, we threshold the UCM image with a value
...
leaf nodes. We use the output of the SVM as the confidence (not binary),
...
requires runtime predictions, and thus cannot take advantage of this preprocessing
step.
...
histogram intersection between the canonical distribution and the
...
regions, either positive or negative. We combine all 
positive regions while removing all negative ones to form the
final segmentation. We use three thresholds for the similarity
...
the segmentation is perfect for the object, we cut all descendent nodes
and return this segment. If the similarity is below
$thre\_neg$, which means the segment does not contain the object, we
cut all descendent nodes and return NULL. If the similarity is
between $thre\_perf$ and $thre\_pos$, which means the node
definitely contains the object but the segmentation is not perfect
yet, we check its children nodes to refine the
segmentation. The final case is that the similarity is between
$thre\_pos$ and $thre\_neg$, which means we are not sure whether
there is an object in the node, so we just search its
children.

The object classification is based on the output of ADDITIVE\_PRUNE. If the output is not NULL, which means at least one node in the tree has similarity above $thre\_pos$, then the object is detected in the image.
...
the other three baseline methods. LowFea means that we use the low level features to train and test 
...
such as the second image in figure \ref{fig:horse_bad_results} removes the object of interest
(horse, in this case) and almost nothing else.  These errors are caused by two things:
first, the additive algorithm currently does not use all levels in the segmentation tree,
as it would make testing take prohibitively long.
...
don't have enough features to represent their attributes. Second, the training of attributes based on
bounding boxes is not accurate, because many background pixels can be included in the bounding boxes. 
...
the more difficult ones, at least part of the object was present in the final
...
under-represented or ill-trained attributes.
...
boxes as ground truth can introduce many background features into the attribute models. The extreme example is shown in the left image of figure \ref{fig:horse_bad_results}, 
...
the attributes of wheel or metal only based on car images, we may get the car model rather than 
the attributes model. One way to deal with this problem is to directly label attributes' 
ground truth. Then we can get similar accurate attribute models as the object models, although 
it would be a tremendously time-consuming task to manually segment all attributes' regions.
...
The holy grail of our approach, of course, is to be able to segment any object
