...
confidences in prediction of object-related attributes were generally low. The bounding 
boxes groundtruth can include many background features into the attribute models. Those 
features are very harmful. The extreme example is shown in the left image of figure \ref{fig:horse_bad_results}, 
which only segments out the background grass. However, even if we have the segmentation 
of objects as the groundtruth for training the attributes, we may still not have accurate 
models, since the groundtruth is for objects not for attributes. For example, if we train 
...
groundtruth. Then we can get similar accurate attribute models as the object models. 
